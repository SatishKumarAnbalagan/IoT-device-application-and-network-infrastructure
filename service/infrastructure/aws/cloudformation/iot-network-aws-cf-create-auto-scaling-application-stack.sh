...
##Creating WAF Stack
response_waf=$(aws cloudformation create-stack --stack-name iot-nw-waf-rules --template-body file://owasp_10_base.yml)
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo "Waiting for Stack waf-rules to be created"
echo "$response_waf"

aws cloudformation wait stack-create-complete --stack-name iot-nw-waf-rules
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
response=$(aws cloudformation create-stack --stack-name "$1" --template-body file://iot-network-aws-cf-auto-scaling-application.json --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey="KEYNAME",ParameterValue=$KEY_CHOSEN ParameterKey="AMIID",ParameterValue=$amiId ParameterKey="BUCKETNAME",ParameterValue=$s3BucketName ParameterKey="APPNAME",ParameterValue="iotnw-webapp" ParameterKey="DEPGROUPNAME",ParameterValue="iotnw-webapp-deployment" ParameterKey="CERTIFICATEARN",ParameterValue=$certificate_arn ParameterKey="HOSTEDZONE",ParameterValue=$hostedZone ParameterKey="HOSTEDZONEID",ParameterValue=$hostedZoneId)
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
